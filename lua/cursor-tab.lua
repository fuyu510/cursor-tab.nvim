...
function M.trim_existing_context(bufnr, range_replace, suggestion, request_line)
	...
	local max_inside_prefix = #current_lines
	if request_line and request_line >= range_replace.start_line and request_line <= range_replace.end_line then
		max_inside_prefix = request_line - range_replace.start_line
	end

	local prefix = 0
	while prefix < max_inside_prefix and prefix < #suggestion_lines do
